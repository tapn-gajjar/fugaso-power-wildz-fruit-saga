...
export class MainView extends MainViewBase {
    constructor() {
        ...
        this.getChildByName("c_logo").getChildByName("a_logo").parentGroup = AppG.stage.mainView;
    }
    ...
    _createGraphic() {
        ...
        /** @type {OMY.OContainer} */
        this._reelWildLandCanvas = this.getChildByName("reels").getChildByName("c_wildlandeffect");
        this._reelWildLandCanvas.setAll("visible", false);

        this._editModeWildLandCanvas = this._reelWildLandCanvas.json["edit"];
        if (this._editModeWildLandCanvas) {
            this._activeWildLandEffect = this._reelWildLandCanvas.getChildByName("reel_2");
            this._activeWildLandEffect.visible = true;
            this._activeWildLandEffect.gotoAndPlay(0, true);
        }

        this._editMode = this._reelWaitCanvas.json["edit"];
        if (this._editMode) {
            this._activeWaitEffect = this._reelWaitCanvas.getChildByName("reel_4");
        ...
        if (this._editMode) {
            this._effect1 = OMY.Omy.add.actorJson(this._wildsCanvas, this._wildsCanvas.json["wild_1"]);
            this._effect2 = OMY.Omy.add.actorJson(this._wildsCanvas, this._wildsCanvas.json["wild_2"]);
            this._effect3 = OMY.Omy.add.actorJson(this._wildsCanvas, this._wildsCanvas.json["wild_3"]);
            this._effect1.gotoAndPlay(0, true, this._effect1.json["idle"]);
            this._effect2.gotoAndPlay(0, true, this._effect2.json["idle"]);
            this._effect3.gotoAndPlay(0, true, this._effect3.json["idle"]);
        }
        ...
        this._editMode = this._reelWaitFrameCanvas.json["edit"];
        if (this._editMode) {
        ...
        if(!AppG.isBeginRespin){
            let effectstartName;
            this._startWildEffects = [];
            this._editSpines = [];
            for (let i = 0; i < this._activeList.length; i++) {
                for (let j = 0; j < this._activeList[0].length; j++) { 
                    if (this._activeList[i][j].symbolName === "H") {
                        effectstartName = "wild_" + String(i);
                        this._editSpines[i] = OMY.Omy.add.actorJson(this._wildsCanvas, this._wildsCanvas.json[effectstartName]); 
                        // this._spines[j].name = String(i) + "_" + String(j); 
                        this._activeList[i][j].respinSymbol(this._editSpines[j]);
                    }
                }
            }
            ...
            for (let i = 0; i < this._activeList.length; i++) {
                for (let j = 0; j < this._activeList[0].length; j++) {  
                    if (this._activeList[i][j].symbolName === "H") {
                        effectstartName = "wild_" + String(i);
                        this._startWildEffects[i] = OMY.Omy.add.actorJson(this._wildsCanvas, this._wildsCanvas.json[effectstartName]);
                        this._startWildEffects[i].gotoAndPlay(0, true, this._startWildEffects[i].json["idle"]);
                        break;
                    }
                }           
            }
    ...
    sendSpin() {
        ...
        if(this._activeWildLandEffect1){
            OMY.Omy.remove.tween(this._activeWildLandEffect1);
            OMY.Omy.add.tween(this._activeWildLandEffect1, {alpha: 0, onCompleteParams: [this._activeWildLandEffect1]},
                this._reelWaitCanvas.json["alpha_time"], (spine) => {
                    spine.stop();
                    spine.visible = false;
                });
            this._activeWildLandEffect1 = null;
        }
        if(this._activeWildLandEffect2){
            OMY.Omy.remove.tween(this._activeWildLandEffect2);
            OMY.Omy.add.tween(this._activeWildLandEffect2, {alpha: 0, onCompleteParams: [this._activeWildLandEffect2]},
                this._reelWaitCanvas.json["alpha_time"], (spine) => {
                    spine.stop();
                    spine.visible = false;
                });
            this._activeWildLandEffect2 = null;
        }
        if(this._activeWildLandEffect3){
            OMY.Omy.remove.tween(this._activeWildLandEffect3);
            OMY.Omy.add.tween(this._activeWildLandEffect3, {alpha: 0, onCompleteParams: [this._activeWildLandEffect3]},
                this._reelWaitCanvas.json["alpha_time"], (spine) => {
                    spine.stop();
                    spine.visible = false;
                });
            this._activeWildLandEffect3 = null;
        }
    ...
    _onReelEaseStops(reelId) {
        ...
        if (AppG.isBeginRespin || AppG.isRespin) {
            if (Boolean(AppG.serverWork.newHoldReel[reelId])) {
                for (let j = 0; j < this._activeList[reelId].length; j++) {
                    if (this._activeList[reelId][j].symbolName === "H") {
                        this.playWildLandEffect(reelId);
                        this._activeList[reelId][j].playWildWaitEffect();
                        break;
                    }
                }
    ...
    playWildLandEffect(reelId){
        if(reelId == 1){
            this._activeWildLandEffect1 = this._reelWildLandCanvas.getChildByName("reel_" + String(reelId + 1));
            this._activeWildLandEffect1.visible = true;
            this._activeWildLandEffect1.alpha = 0;
            this._activeWildLandEffect1.gotoAndPlay(0, true);
            OMY.Omy.add.tween(this._activeWildLandEffect1, {alpha: 1},
                this._reelWildLandCanvas.json["alpha_time"]);
        }else if(reelId == 2){
            this._activeWildLandEffect2 = this._reelWildLandCanvas.getChildByName("reel_" + String(reelId + 1));
            this._activeWildLandEffect2.visible = true;
            this._activeWildLandEffect2.alpha = 0;
            this._activeWildLandEffect2.gotoAndPlay(0, true);
            OMY.Omy.add.tween(this._activeWildLandEffect2, {alpha: 1},
                this._reelWildLandCanvas.json["alpha_time"]);
        }else if(reelId == 3){
            this._activeWildLandEffect3 = this._reelWildLandCanvas.getChildByName("reel_" + String(reelId + 1));
            this._activeWildLandEffect3.visible = true;
            this._activeWildLandEffect3.alpha = 0;
            this._activeWildLandEffect3.gotoAndPlay(0, true);
            OMY.Omy.add.tween(this._activeWildLandEffect3, {alpha: 1},
                this._reelWildLandCanvas.json["alpha_time"]);
        }
    }
...
//-------------------------------------------------------------------------
    ...
    changeReelForWild() {
        ...
        this.expandsSoundExecuted = false;
    ...
    _playReelChangeEffect(i) {
        ...
        this._wildEffects = [];   
        for (let j = 0; j < i; j++) {     
            effectName = "wild_" + String(j+1);
            this._wildEffects[j] = OMY.Omy.add.actorJson(this._wildsCanvas, this._wildsCanvas.json[effectName]);
        }
        this._spines = [];
        for (let j = 0; j < this._activeList[i].length; j++) {
            ...
            if (countWild === 3) {
                ...
                if(startPoint){
                    this._spines[j].setXY(startPoint.x, startPoint.y);
                }                
            }
            ...
            if (this._activeList[i][j].symbolName === "H") {
                ...
                OMY.Omy.add.tween(this._spines[j], {x: point.x , y: point.y}, 0.5, this._changeSymbol2Respin.bind(this),
                {
                    delay: 0,
        ...
        for (let i = 0; i < this._spines.length; i++) {
            this._spines[i].gotoAndPlay(0, false, this._spines[i].json["expend"]);
        }

        this._spines[2].addComplete(() => {
            //Start big wild effect
            this._playWildStart(i);
            let slotWildname;
            for(let j = 0; j < this._activeList[i].length; j++) {
                slotWildname = String(i) + "_" + String(j);
                this._wildsCanvas.getChildByName(slotWildname).alpha = 0;
            }
        }, this, false);
    }
    ...
    /**     * @private     */
    _playWildStart(i) {
        for (let j = 0; j <= this._wildEffects.length; j++) {
            if(i == (j+1)){
                this._wildEffects[j].parentGroup = AppG.stage.mainView;
                this._wildEffects[j].gotoAndPlay(0, false, this._wildEffects[j].json["start"]);
                if(!this.expandsSoundExecuted){
                    OMY.Omy.sound.play(GameConstStatic.S_wild_expands);
                    this.expandsSoundExecuted = true;
                }
                
                this._wildEffects[j].addComplete(() => {
                    this._wildEffects[j].gotoAndPlay(0, true, this._wildEffects[j].json["idle"]);
                }, this, false);
            }
        }
    }
...
//-------------------------------------------------------------------------
    ...
    showWinCombo() {
        ...
        if (AppG.winCoef > 5) OMY.Omy.sound.play(GameConstStatic.S_show_win_5);
        else if ((AppG.winCoef > 1) && (AppG.winCoef <= 5)) OMY.Omy.sound.play(GameConstStatic.S_show_win_4);
        else if (AppG.winCoef <= 1) OMY.Omy.sound.play(GameConstStatic.S_show_win_3);
